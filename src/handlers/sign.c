    ...
    switch (txContent.contractType) {
        ...
        case TRIGGERSMARTCONTRACT:   // TRC20 Transfer
            ...
            if (txContent.contractType == TRIGGERSMARTCONTRACT) {
                ...
                if (!tostring256(&uint256, 10, (char *) G_io_apdu_buffer + 100, 100)) {
                    return io_send_sw(E_INCORRECT_LENGTH);
                }
        ...
        case WITNESSUPDATECONTRACT: {
            size_t url_length = msg.witness_update_contract.update_url.size;
            if (url_length == 0 || url_length >= sizeof(witnessUrl)) {
                return io_send_sw(E_INCORRECT_DATA);
            }
            memcpy(witnessUrl, msg.witness_update_contract.update_url.bytes, url_length);
            witnessUrl[url_length] = '\0';
