...
static bool handle_above_threshold_msg(const tlv_data_t *data,
    ...
    return tlv_get_printable_string(data,
                                    context->param->above_threshold_msg,
                                    1U,
                                    sizeof(context->param->above_threshold_msg));
}
...
static bool process_token_amount(const s_param_token_amount *param,
    ...
    if (param->has_token) {
        if (!parsed_value_to_address(token, addr_buf)) {
            return false;
        }
...
bool format_param_token_amount(const s_param_token_amount *param, const char *name) {
    ...
    if (param->has_token && (param->token.type_family != TF_ADDRESS)) {
        return false;
    }
