...
- `fuzz_tip712_wallet`: builds the same TIP712 harness with
  `SCREEN_SIZE_WALLET` enabled so wallet-screen conditional branches are
  sanitizer-covered too.
...
for SDK, UI, settings, and signature-verification dependencies. The host build
now shares the same BIP32-path parsing logic as production `read_bip32_path_712`
instead of using a separate host-only parser.

TIP712-specific coverage goals for the host harness now include:

- production BIP32 path parsing for `TIP712_SIGN`
- BASIC-mode runs with `S_SIGN_BY_HASH` and `S_VERBOSE_TIP712`
- FULL filtering flows for raw, trusted-name, and amount-join formatting
- `SCREEN_SIZE_WALLET` compile-time branches via `fuzz_tip712_wallet`
- semantic seed coverage for nested arrays, empty arrays, trusted-name hit and
  fallback, amount-join, datetime, signed-int, `P1_PARTIAL` chunked payloads,
  permit-style `token_idx == 0xFF` token resolution, and reset/replay flows
- sanitizer-backed coverage for the legacy `SIGN_TIP_712_MESSAGE` handler via
  `fuzz_tip712_legacy`

The checked-in TIP712 seed corpus lives under [`corpus/fuzz_tip712`](./corpus/fuzz_tip712).
It is generated by [`generate_tip712_corpus.py`](./generate_tip712_corpus.py),
which converts a small set of curated JSON/regression scenarios into the compact
`fuzz_tip712` stream format.
The legacy TIP712 replay corpus lives under
[`corpus/fuzz_tip712_legacy`](./corpus/fuzz_tip712_legacy).

The host fuzz environment still uses deterministic stubs for NBGL transitions,
approval flows, and certificate/signature verification. These targets aim to
close the highest-value corpus and handler coverage gaps, not to emulate the
full device UI or cryptographic verification stack.
...
To build both TIP712 variants explicitly:

```sh
cd tests/fuzzing
cmake -B build -S . -DCMAKE_C_COMPILER=/usr/bin/clang -DSANITIZER=address
cmake --build build --target fuzz_tip712 fuzz_tip712_wallet
```

To build all current TIP712 targets, including the legacy handler target:
...
docker run --platform linux/amd64 --rm --privileged \
  ...
  -v "$(pwd)/tests/fuzzing/out:/out" \
  app-tron-fuzz \
  /bin/bash -lc 'cd /src/app-tron/tests/fuzzing && rm -rf build && cmake -B build -S . && cmake --build build --target fuzz_tip712 fuzz_tip712_wallet && cp ./build/fuzz_tip712 ./build/fuzz_tip712_wallet /out/'
```
...
For open-ended parallel fuzzing, keep the same two `run_fuzzer` commands and
replace `-- -runs=10000 -max_len=8192` with `-- -max_len=8192`.

Notes:

- The CMake build can produce both `transaction_trigger_decode_fuzzer` and
  both TIP712 variants, but the default `.clusterfuzzlite/build.sh` currently exports
  only `transaction_trigger_decode_fuzzer` to `tests/fuzzing/out`.
- The checked-in TIP712 corpus is intentionally target-specific:
  `tests/fuzzing/corpus/fuzz_tip712` is used for both `fuzz_tip712` and
  `fuzz_tip712_wallet`.
