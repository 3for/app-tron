#pragma once
// Host-side mock of the SDK <base58.h>. Ports the real lib_standard_app base58
// encode/decode (verbatim algorithm) so the fuzz build exercises the genuine
// Base58 / Base58Check paths: CAL descriptors now carry TRON Base58 addresses that
// must decode and checksum-verify, so a placeholder decoder would give zero coverage.
#include <stddef.h>
#include <stdint.h>
#include <string.h>
...
static inline int base58_decode(const char *in, size_t in_len, uint8_t *out, size_t out_len) {
    static const uint8_t BASE58_TABLE[] = {
    ...
    for (uint8_t i = 0; i < in_len; i++) {
        if ((uint8_t) in[i] >= sizeof(BASE58_TABLE)) {
            return -1;
        }
...
static inline int base58_encode(const uint8_t *in, size_t in_len, char *out, size_t out_len) {
