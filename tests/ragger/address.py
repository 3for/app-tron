        ...
        if len(raw_addr) == 42:
            if raw_addr.startswith("0x"):  # eth address format
                return base58.b58encode_check(
                    b"\x41" + bytes.fromhex(raw_addr[2:])).decode()
...
def is_base58check_address(value: str) -> bool:
    try:
        return value.startswith("T") and len(
            base58.b58decode_check(value)) == 21
    except Exception:
        return False
...
def is_address(value: str) -> bool:
    return is_base58check_address(value) or is_hex_address(
        value) or is_eth_address(value)
