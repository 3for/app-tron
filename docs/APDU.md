...
- [`src/apdu_constants.h`](../src/apdu_constants.h) and status words in
...
| `0x14` | `INS_PROVIDE_NFT_INFORMATION` | metadata handler            | Provide signed NFT metadata              |
| `0x1A` | `INS_TIP712_STRUCT_DEF`      | TIP-712 handler              | Define a full TIP-712 structure          |
| `0x1C` | `INS_TIP712_STRUCT_IMPL`     | TIP-712 handler              | Stream full TIP-712 values               |
| `0x1E` | `INS_TIP712_FILTERING`       | TIP-712 handler              | Provide signed TIP-712 filters           |
| `0x20` | `INS_GET_CHALLENGE`          | metadata handler             | Get a metadata-signing challenge         |
| `0x22` | `INS_PROVIDE_TRUSTED_NAME`   | metadata handler             | Provide signed trusted-name metadata     |
| `0x24` | `INS_PROVIDE_ENUM_VALUE`     | metadata handler             | Provide signed enum metadata             |
| `0x26` | `INS_GTP_TRANSACTION_INFO`   | generic parser               | Provide generic transaction information |
| `0x28` | `INS_GTP_FIELD`              | generic parser               | Provide a generic transaction field      |
| `0x2A` | `INS_PROVIDE_PROXY_INFO`     | metadata handler             | Provide signed proxy metadata            |
| `0x38` | `INS_PROVIDE_GATING`         | metadata handler             | Provide a signed gating descriptor       |
| `0xC8` | `INS_SIGN_PERSONAL_MESSAGE_FULL_DISPLAY` | message handler | Review and sign a displayed message |
| `0xCA` | `INS_PROVIDE_TRC20_TOKEN_INFORMATION` | metadata handler | Provide signed TRC20 metadata       |
| `0xD4` | `INS_SIGN_GCS`               | `handleSignGCS`              | Generic Clear Signing store/start flow   |
...
Returns the active settings flags and the application version. `P1` and `P2` must be `0x00` and
the command data must be empty.

**Response** (4 bytes)

```
[1 byte] settings and capability flags (see below)
...
| --- | --------------------------- | -------------------------------------- |
| 0   | `APP_FLAG_DATA_ALLOWED`     | Allow contracts carrying `data`        |
| 1   | `APP_FLAG_CUSTOM_CONTRACT`  | Allow arbitrary smart-contract calls   |
| 2   | `APP_FLAG_TRUNCATE_ADDRESS` | Reserved; deprecated and always clear  |
| 3   | `APP_FLAG_SIGN_BY_HASH`     | Allow blind/hash-only signing          |
| 4   | `APP_FLAG_VERBOSE_TIP712`   | Use verbose TIP-712 review             |
| 5   | `APP_FLAG_DISPLAY_HASH`     | Include the transaction hash in review |
| 6   | `APP_FLAG_TIP712_FILTER_V2` | Full TIP-712 filter-v2 capability      |
...
across multiple chunks. This legacy hash-only review requires the **Sign by hash** setting;
`INS_SIGN_PERSONAL_MESSAGE_FULL_DISPLAY` (`0xC8`) is the clear-review alternative.
...
| `0x6F00` | `E_INTERNAL_ERROR`                  | Internal error                                    |
